...
class _MenuVoteScreenState extends State<MenuVoteScreen> {
  ...
  Map<String, Map<String, List<Map<String, dynamic>>>> _menuConfigurations = {};
  ...
  // Load menu configurations from Firestore
  Future<void> _loadMenuConfigurations() async {
    try {
      setState(() {
      ...
      Map<String, Map<String, List<Map<String, dynamic>>>> configurations = {};
      ...
      for (var doc in snapshot.docs) {
        ...
        if (configurations[day] == null) {
          configurations[day] = {};
        }
        configurations[day]![mealType] = options;
      }

      // Ensure all days and meal types have at least empty configurations
      for (String day in days) {
        if (configurations[day] == null) {
          configurations[day] = {};
        }
        for (String mealType in ['breakfast', 'lunch', 'dinner']) {
          if (configurations[day]![mealType] == null) {
            configurations[day]![mealType] = [];
          }
        }
      }

      setState(() {
        _menuConfigurations = configurations;
        _isLoadingConfig = false;
      });
    } catch (e) {
      print('Error loading menu configurations: $e');
      setState(() {
        _isLoadingConfig = false;
      });
    }
  ...
  Future<void> _saveMenuConfiguration(String day, String mealType, List<Map<String, dynamic>> options) async {
    try {
      final docId = '${day}_$mealType';
      await FirebaseFirestore.instance
          .collection('voting_menu_options')
          .doc(docId)
          .set({
        ...
        'updatedBy': _currentUserData?['name'] ?? 'Admin',
      });

      // Update local state
      setState(() {
        if (_menuConfigurations[day] == null) {
          _menuConfigurations[day] = {};
        }
        _menuConfigurations[day]![mealType] = options;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${getLocalizedMealType(context, mealType)} options for ${getLocalizedDay(context, day)} saved successfully'),
          backgroundColor: Colors.green,
        ),
      );
    } catch (e) {
      print('Error saving menu configuration: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              ElevatedButton.icon(
                onPressed: _loadMenuConfigurations,
                style: ElevatedButton.styleFrom(
          ...
          Text(
            'Configure voting options for each day and meal type. These options will be available for users to vote on.',
            style: TextStyle(
              fontSize: 14,
          ...
          // Day selection for configuration
          Container(
          ...
          // Configuration panels for each meal type
          if (_isLoadingConfig)
            const Center(
              child: Padding(
            ...
            Column(
              children: [
                _buildMealConfigPanel('breakfast'),
                const SizedBox(height: 15),
                _buildMealConfigPanel('lunch'),
                const SizedBox(height: 15),
                _buildMealConfigPanel('dinner'),
              ],
            ),
        ],
      ),
  ...
  // Build individual meal configuration panel
  Widget _buildMealConfigPanel(String mealType) {
    final options = _menuConfigurations[_configSelectedDay]?[mealType] ?? [];
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(
                  '${getLocalizedMealType(context, mealType)} Options',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.orange.shade700,
                ...
                const Spacer(),
                ElevatedButton.icon(
                  onPressed: () => _showAddOptionDialog(_configSelectedDay, mealType),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.green,
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Add options that users can vote for',
                      style: TextStyle(
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Row(
                        ...
                        children: [
                          IconButton(
                            onPressed: () => _showEditOptionDialog(_configSelectedDay, mealType, index, option),
                            icon: const Icon(Icons.edit, color: Colors.blue),
                            tooltip: 'Edit',
                          ),
                          IconButton(
                            onPressed: () => _deleteOption(_configSelectedDay, mealType, index),
  ...
  void _showAddOptionDialog(String day, String mealType) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Add ${getLocalizedMealType(context, mealType)} Option'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'e.g., Spicy chicken curry with rice',
                border: OutlineInputBorder(),
              ),
              maxLines: 2,
            ),
          ...
          ElevatedButton(
            onPressed: () {
              if (nameController.text.trim().isNotEmpty) {
                _addNewOption(day, mealType, nameController.text.trim(), descriptionController.text.trim());
  ...
  // Show dialog to edit existing option
  void _showEditOptionDialog(String day, String mealType, int index, Map<String, dynamic> option) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Edit ${getLocalizedMealType(context, mealType)} Option'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (nameController.text.trim().isNotEmpty) {
                _editOption(day, mealType, index, nameController.text.trim(), descriptionController.text.trim());
                Navigator.pop(context);
              }
            },
  ...
  // Add new option
  void _addNewOption(String day, String mealType, String name, String description) {
    final options = List<Map<String, dynamic>>.from(_menuConfigurations[day]?[mealType] ?? []);
    options.add({
    ...
    _saveMenuConfiguration(day, mealType, options);
  }

  // Edit existing option
  void _editOption(String day, String mealType, int index, String name, String description) {
    final options = List<Map<String, dynamic>>.from(_menuConfigurations[day]?[mealType] ?? []);
  ...
  // Delete option
  void _deleteOption(String day, String mealType, int index) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
